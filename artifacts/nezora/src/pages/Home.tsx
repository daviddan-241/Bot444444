  ...
  return (
    <Shell>
      ...
      <section className="px-5">
        <div className="rounded-[36px] p-6 text-white shadow-soft" style={{ background: '#07111F' }}>
          ...
          <p className="mt-4 text-sm leading-6" style={{ color: 'rgba(255,255,255,0.7)' }}>Host Nezora on Render, unlock it with your personal token, upload a ZIP or connect a GitHub repo, then deploy through real provider flows.</p>
          <div className="mt-6 grid grid-cols-3 gap-3">
            {[['Docker', 'runtime'], ['ZIP', 'uploads'], ['HTTPS', 'provider URLs']].map(([a, b]) => (
              <div key={b} className="rounded-3xl p-3 text-center" style={{ background: 'rgba(255,255,255,0.10)' }}>
                <p className="text-lg font-black">{a}</p>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.60)' }}>{b}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-6 grid grid-cols-2 gap-3 px-5 pb-6">
...
function Action({ href, icon, title, desc, primary }: { href: string; icon: React.ReactNode; title: string; desc: string; primary?: boolean }) {
  return (
    <Link href={href} className="flex items-center justify-between rounded-[30px] p-4 shadow-soft" style={{ background: primary ? '#0A84FF' : '#fff', color: primary ? '#fff' : '#07111F', outline: primary ? '1px solid #0A84FF' : '1px solid #E7ECF3', boxShadow: '0 18px 50px rgba(7,17,31,0.08)' }}>
      <div className="flex items-center gap-3">
        <div className="grid h-12 w-12 place-items-center rounded-2xl" style={{ background: primary ? 'rgba(255,255,255,0.15)' : '#EEF6FF', color: primary ? '#fff' : '#006BE6' }}>{icon}</div>
        <div>
          <h3 className="font-black">{title}</h3>
          <p className="mt-1 text-sm leading-5" style={{ color: primary ? 'rgba(255,255,255,0.75)' : '#65758B' }}>{desc}</p>
        </div>
      </div>
      <ArrowRight />
    </Link>
...
function Path({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="flex gap-3 rounded-3xl p-4" style={{ background: '#F6F8FB' }}>
      <div style={{ color: '#006BE6' }}>{icon}</div>
      <div>
...
function Feature({ icon, title }: { icon: React.ReactNode; title: string }) {
  return (
    <div className="rounded-[28px] bg-white p-4 shadow-soft ring-1" style={{ boxShadow: '0 18px 50px rgba(7,17,31,0.08)', outline: '1px solid #E7ECF3' }}>
